      ...
      <div className="relative z-10 container mx-auto pt-32 pb-20 px-6 lg:px-12">
        ...
        <div className="mb-12 border-l-4 border-[#BF092F] pl-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black text-[#44444E] uppercase tracking-[0.4em]">Data Archive</span>
          </div>
          <h1 className="text-4xl md:text-5xl font-black text-[#44444E] uppercase tracking-tighter">
            {results.length > 0 ? `Results for: ${decodedQuery}` : `No Records Found`}
          </h1>
        </div>
        ...
        {results.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-t border-l border-gray-200">
            {results.map((item, idx) => (
              <div
                key={idx}
                className="group bg-white border-r border-b border-gray-200 p-8 transition-all duration-300 hover:bg-[#44444E] cursor-pointer"
                onClick={() => navigate(`/products/${item.categorySlug}/${encodeURIComponent(item.name)}`)}
              >
                {/* Image Component */}
                <div className="relative h-60 w-full overflow-hidden bg-gray-50 border border-gray-100 mb-8">
                  <img
                    src={item.images[0]}
                    alt={item.name}
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black text-[#BF092F] uppercase tracking-[0.2em]">Product Index: 0{idx + 1}</span>
                    <ShieldCheck size={16} className="text-gray-200 group-hover:text-[#BF092F] transition-colors" />
                  </div>
                  ...
                  <div className="pt-6 flex items-center justify-between border-t border-gray-100 group-hover:border-white/10 mt-4">
                    <div className="flex items-center gap-2">
                       ...
                       <span className="text-[9px] font-black text-gray-400 group-hover:text-white/40 uppercase">Technical Specs</span>
                    </div>
                    <ArrowRight size={18} className="text-[#44444E] group-hover:text-white group-hover:translate-x-2 transition-all" />
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="mt-12 flex justify-between items-end border-t border-gray-100 pt-8">
           ...
           <div className="text-[9px] font-mono text-gray-400 uppercase tracking-widest text-right">
             Search Operation {results.length > 0 ? `Results for: ${decodedQuery}` : `No Records Found`}
           </div>
        </div>
      </div>
    </div>
